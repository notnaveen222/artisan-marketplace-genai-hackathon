  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y =
        position[1] + Math.sin(state.clock.getElapsedTime() * 0.5) * 0.2;
    }
  });
...
function FloatingFabric({ position = [0, 0, 0] as [number, number, number] }) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [hovered, setHovered] = React.useState(false);
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.z =
        Math.cos(state.clock.getElapsedTime() * 0.2) * 0.2;
      meshRef.current.position.y =
        position[1] + Math.sin(state.clock.getElapsedTime() * 0.4) * 0.3;
    }
  });
...
function FloatingJewelry({ position = [0, 0, 0] as [number, number, number] }) {
...
function AINetwork() {
  ...
  const nodes = useMemo(() => {
    const temp: Array<{ position: [number, number, number]; scale: number }> =
      [];
    for (let i = 0; i < 20; i++) {
          ...
          <line key={`line-${i}`}>
            <bufferGeometry>
              <bufferAttribute
                ...
                count={2}
                array={
                  new Float32Array([...node.position, ...nextNode.position])
                }
                itemSize={3}
                args={[
                  new Float32Array([...node.position, ...nextNode.position]),
...
function WoodenCarving({ position = [0, 0, 0] as [number, number, number] }) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [hovered, setHovered] = React.useState(false);
...
function ParticleField() {
  const particles = useMemo(() => {
    const temp: Array<{ position: [number, number, number]; size: number }> =
      [];
    for (let i = 0; i < 300; i++) {
      temp.push({
        position: [
